    ...
    <div class="card rounded p-4 mb-5">
        <div class="row row-cols-1 row-cols-md-1 row-cols-lg-2 row-cols-xl-3 g-4">
            @foreach ($SoftwareList as $Software)
                <div class="col">
                    <div class="card rounded h-100 shadow" style="background-color: #E8F1F5">
                        <div class="d-flex justify-content-center align-items-center p-3">
                            <img src="{{ $Software->SoftwareImage }}" width="120px" height="120px"
                                alt="{{ $Software->SoftwareName }}">
                        </div>
                        <div class="card-body d-flex flex-column d-sm-block text-white" style="background-color: #015791">
                            <h5 class="card-title fw-bold text-truncate">{{ $Software->SoftwareName }}</h5>
                            <p class="card-text text-truncate-2">{{ $Software->SoftwareDescription }}</p>
